...
class DynamicProgramWithSolarCurtailmentController(BatteryController):
    """
    Optimal battery control using dynamic programming
    """

    def __init__(self, name='DynamicProgramWithSolarCurtailmentController', params=None, debug=False):
        """
    ...
    def _run_dynamic_program(self):
        ...
        for col in range(self.num_time_intervals - 2, -1, -1):
            ...
            # Determine how much solar curtailment is possible in this interval
            # num_rows_curtailment = 0
            # if self.params['allow_solar_curtailment']:
            #    if self.tariff_export[col] < 0:
            #        num_rows_curtailment = round(self.generation[col] / self.params['soc_interval'] * 100)
            ...
            for row in range(0, self.num_soc_intervals):
                ...
                # prev_row_max = int(min(self.num_soc_intervals - 1,
                #                       row + self.max_soc_increase_interval + num_rows_curtailment))
                ...
                for prev_row in range(prev_row_min, prev_row_max + 1):
                    ...
                    if self.params['allow_solar_curtailment'] and \
                            (net_grid_impact_kw < 0) and \
                            (self.tariff_export[col] < 0):
                        #print("curtailing")
                        #print(" - from", net_grid_impact_kw, "kW (", net_grid_impact_kwh, "kWh)")
                        this_solar_curtailment = min(-1*net_grid_impact_kw, self.generation[col])
                        #print(" - curtailing", this_solar_curtailment)
                        net_grid_impact_kw = net_grid_impact_kw + this_solar_curtailment
                        net_grid_impact_kwh = net_grid_impact_kw * self.time_interval_in_hours
                        #print(" - to", net_grid_impact_kw, "kW (", net_grid_impact_kwh, "kWh)")

                    #if col == 19:
                    #    print(self.demand[col],
                    #          self.generation[col],
                    #          battery_impact_kwh,
                    #          this_solar_curtailment,
                    #          net_grid_impact_kwh,
                    #          net_grid_impact_kw)
                    ...
                    if net_grid_impact_kwh > 0:
                        ...
                        # (When export tariff == market price, this is irrelevant.)
